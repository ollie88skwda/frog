// Vectorizes the frog character reference photo into a clean flat-color SVG
...
// fill, white eye-highlight dots) underneath JPEG compression noise and
// anti-aliasing, so tracing each color plane separately and letting potrace
// fit clean bezier curves reproduces it far more faithfully than a generic
// multi-color autotrace (imagetracerjs was tried first and rejected: only
// 21.8% pixel-exact, visibly faceted edges — this color-separated approach
// gets to 94% of pixels within a diff of 20/255, the rest a sub-pixel edge
// halo, not shape error — see docs/DECISIONS.md 2026-08-05).
//
...
// In-page: key the white background to alpha, trim to content bbox, classify
// every opaque pixel into one of 3 flat-color buckets, despeckle each mask
// (3x3 majority vote — kills isolated JPEG-noise pixels), and pack each into
// a raw P4 PBM buffer (1 bit/pixel, MSB-first, 1 = black/foreground, the
// convention potrace expects). Returned as base64 so Node never needs a PNG
// decoder.
const result: { width: number; height: number; pbm: Record<string, string> } =
  await page.evaluate(async (src) => {
    ...
    // Alpha-key (distance-from-white) + trim bbox — same threshold band as
    // gen-pwa-icons.ts's cutout step.
    ...
    // Despeckle: 3x3 majority vote kills isolated JPEG-noise pixels without
    // eroding real edges.
