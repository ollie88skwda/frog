...
- **PostgREST caps every `select()` at 1000 rows by default** — a flat select on a table that can grow past that (exercise library, set logs, …) silently truncates rather than erroring. Any query over a table without a caller-supplied `limit`/`range` must paginate via `SupabaseRepo`'s private `selectAll()` helper (`packages/core/src/repo/supabase.ts`), which loops `.range()` until a short page signals the end — see `listExercises()`/`exportAll()` for the pattern. Order by a stable column (e.g. `id`) when paginating so rows aren't skipped or repeated across pages.
