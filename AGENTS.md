...
**Brand mark.** `apps/web/public/icon.svg` is the single source for the shipped assets: every PNG in `apps/web/public/` is a render of it via `bun scripts/gen-pwa-icons.ts` (re-run after any edit, and never hand-edit the PNGs or trace a raster). The brand spec inlines its own copies of the geometry so its samples can be styled and themed — re-sync those by hand when the mark changes. **The tiles show an *icon cut*, not the whole mark** — a square window on the head + body with the haunches, feet and ground bar bleeding off the sides, because the mark is 1.6:1 and centring all of it leaves the frog floating (see `docs/DECISIONS.md` → "icon cut"). The generator derives that window from the mark's measured geometry, so it follows the frog; it also asserts `icon.svg`'s own `viewBox` and `#pad` translate still match, and prints the replacements if they don't — that assert firing is the expected way to learn the cut moved. The tile ships on the lab-ink ground `#101211` with its line work inverted to sage `#ECEEED` — **ground and line work move together**; flipping the ground alone makes the outline, feet and ground bar vanish and the frog reads as a floating blob (`GROUND` in the generator must match the `#ground` fill in `icon.svg`). The in-app mark is a separate simplified cut, `apps/web/src/components/frog-mark.tsx` — it must stay `currentColor` + `var(--accent)` so it survives both themes, and must stay vector for the 220 kB bundle gate. Sizing and padding rules are in the brand spec's "The mark" section.
