...
- **PostgREST caps every `select()` at 1000 rows by default** — a flat select on a table that can grow past that (exercise library, set logs, …) silently truncates rather than erroring. Any query over a table without a caller-supplied `limit`/`range` must paginate via `SupabaseRepo`'s private `selectAll()` helper (`packages/core/src/repo/supabase.ts`), which loops `.range()` until the rows collected cover the total row count PostgREST reports — so every paginated select must ask for that total with `.select(cols, { count: "exact" })`, which is what lets the common under-1000-row case finish in a single request instead of paying a trailing empty-page probe. A page without a count still terminates on an *empty* page (correct, just one request slower). It advances by the rows actually returned, never by the requested page size (a short page can just mean the server's `max_rows` is below it) — see `listExercises()`/`exportAll()` for the pattern. The token-API edge function (`supabase/functions/api/index.ts`) is Deno and can't import the repo, so it mirrors the same helper locally; keep the two in step. Order by a stable column when paginating so rows aren't skipped or repeated across pages — prefer a natural column (`created_at`, `started_at`) with an `id` tiebreak, since ids are random uuid v4 and an id-only order shuffles user-visible output like the CSV export.
