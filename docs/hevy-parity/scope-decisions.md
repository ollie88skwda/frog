# Context for SBL ← Hevy feature-parity implementation plan

## Task
Clone the Hevy MOBILE app's feature set into SBL (feature parity, built in SBL's own Linear-style design language — no copying of Hevy assets/copy/prose). User will prune later; err toward inclusion within the scope decisions below.

## User scope decisions (final, from Oliver)
1. **Skip social entirely.** No profiles-of-others, follow graph, feeds, likes/comments, discover, leaderboards, suggested users, contact sync, user search, athlete workouts, comparisons, Hevy Coach. SBL stays personal/single-user. DO keep: own profile/stats screen, share-as-image (shareable workout/stat cards, client-side rendered).
...
4. Implied: NO paywall — everything ungated in SBL (ignore Hevy Pro caps: unlimited routines, unlimited custom exercises, full history ranges, all measurements).
5. Preserve SBL's unique features untouched: conditions tracking + findings correlations, muscle-tier science library, machines (catalog/settings-memory/photos), CSV/JSON export, PAT API, MCP server, Hevy/Fitbit importers.
...
- **SBL current-state inventory**: `/private/tmp/claude-502/-Users-Ollie-Documents-Code-sbl/5074eb6e-0488-4551-976f-65fcaa4042c3/scratchpad/hevy/sbl-inventory.md` (written alongside this file).
- SBL repo: `/Users/Ollie/Documents/Code/sbl` — read `AGENTS.md`, `CLAUDE.md`, `packages/core/src/db/schema.ts`, `apps/web/src/screens/session.tsx`, `apps/web/src/lib/queries.ts` for conventions.

## Hard constraints (from SBL CLAUDE.md/AGENTS.md)
...
- Design: current SBL monochrome/0px-radius/8px-grid theme. NO Hevy visual cloning.

## Known open implementation choices the plan should resolve (recommend, don't ask)
- Exercise library expansion: recommend seeding from free-exercise-db (public-domain, ~870 exercises w/ images + instructions) mapped into SBL's schema + muscle/tier taxonomy; keep SBL's 20 curated seeds as the "classified" core; classification of the rest can be incremental.
...
- Body heat map: SBL already has anatomy muscle taxonomy; needs front/back SVG body diagram component.
