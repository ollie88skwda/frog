...
- `user_prefs` — 1 row/user: firstWeekday, includeWarmupsInStats, defaultRestSec, previousValuesScope (`any|routine`), bodyDiagram, plateConfig jsonb, displayName; **+ bio since 2026-08-08 (profile header edit), see DECISIONS.md**. **Split rule**: semantics-bearing/cross-device → server; pure device behavior (theme, display unit, sounds/volumes, keep-awake, column visibility, smart scrolling, PR-banner toggle) → localStorage (existing `lib/settings.ts` pattern)
...
| post-save summary | overlay on `/history/:id?summary=1`: ordinal, streak (first-of-week), slides (PRs, consistency, overview, exercises + mini heat map), share per slide — **as-shipped deck and share system superseded, see DECISIONS.md 2026-07-30 (share system redesign)** |
| share renderer | canvas → PNG, `navigator.share` + download fallback; used everywhere share appears — shipped as `packages/core/src/share/` + `apps/web/src/lib/share/` + `components/share-sheet.tsx`, **not** the planned single `components/share-card.tsx` (same entry) |
