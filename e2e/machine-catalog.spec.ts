...
test("in-session attach: catalog search attaches a machine to the block", async ({
  ...
  // No machine set → the attach affordance lives in the pre-flight card
  // (visible on the logging path, never buried in a menu).
  await expect(page.getByTestId(`block-${NAME}-setup`)).toBeVisible();
